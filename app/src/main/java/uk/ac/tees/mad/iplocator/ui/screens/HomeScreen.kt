...
import uk.ac.tees.mad.iplocator.model.dataclass.Connection
import uk.ac.tees.mad.iplocator.model.dataclass.Currency
...
import uk.ac.tees.mad.iplocator.model.dataclass.TimeZone
...
fun HomeScreen(navController: NavHostController) {
    ...
    Scaffold(modifier = Modifier.fillMaxSize().nestedScroll(scrollBehavior.nestedScrollConnection),
        topBar = {
        IpLocatorTopAppBar(title = "IP Locator", scrollBehavior = scrollBehavior, navController = navController)
    },
        floatingActionButton = {
            ExtendedFloatingActionButton(onClick = {navController.navigate(Dest.MapScreen) },
                icon = {Icon(imageVector = Icons.Default.Explore, contentDescription = null, modifier = Modifier.size(24.dp))},
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(vertical = 8.dp)) {
                DetailRow(label = "City", value = ipLocation.city)
                DetailRow(label = "Region", value = "${ipLocation.regionName} (${ipLocation.regionCode})")
                DetailRow(label = "Country", value = "${ipLocation.countryName} (${ipLocation.countryCode})")
                DetailRow(label = "Continent", value = "${ipLocation.continentName} (${ipLocation.continentCode})")
                DetailRow(label = "Postal Code", value = ipLocation.zip)
                DetailRow(label = "Coordinates", value = "${ipLocation.latitude}, ${ipLocation.longitude}")
                DetailRow(label = "Capital", value = ipLocation.location.capital)
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(vertical = 8.dp)) {
                ...
                DetailRow(label = "IP Type", value = ipLocation.type)
            }
        }
    }
}
